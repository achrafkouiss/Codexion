...
static int	check_burnout(t_monitor *monitor, size_t i)
{
	...
	if (time_since_compile >= coders[i].inputs->time_to_burnout
		&& counter != (int)coders[i].inputs->number_of_compiles_required)
	{
...
void	*monitor_routine(void *arg)
{
	...
	while (1)
	{
		...
		if (finished_count == (int)monitor->coders[0].inputs->number_of_coders)
			return (NULL);
		i = 0;
		while (i < monitor->coders[0].inputs->number_of_coders)
		{
			if (check_burnout(monitor, i))
				return (NULL);
			i++;
		}
		usleep(1000);
